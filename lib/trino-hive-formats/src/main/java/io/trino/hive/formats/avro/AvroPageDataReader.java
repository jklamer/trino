...
{
    //copied from org.apache.avro.io.BinaryDecoder
            ...
            extends BlockBuildingDecoder
    {
        ...
        protected void decodeIntoBlock(Decoder decoder, BlockBuilder builder)
                throws IOException
        {
            // only possible to read String type when underlying write type is String or Bytes
            // both have same encoding that we can take advantage of
            ...
            extends BlockBuildingDecoder
    {
        ...
        protected void decodeIntoBlock(Decoder decoder, BlockBuilder builder)
                throws IOException
        {
            // only possible to read Bytes type when underlying write type is String or Bytes
            // both have same encoding that we can take advantage of
            long size = decoder.readLong();
            if (size > MAX_ARRAY_SIZE) {
            ...
            extends WriterUnionBlockBuildingDecoder
    {
                ...
                throws IOException
        {
            ...
            if (readUnionEquiv) {
                ...
                super.decodeIntoBlock(index, decoder, builder);
            }
        }
        ...
        protected static int[] getIndexToChannel(List<Schema> schemas)
        {
            ...
            for (int i = 0; i < indexToChannel.length; i++) {
                if (schemas.get(i).getType().equals(Schema.Type.NULL)) {
                    indexToChannel[i] = -1;
                }
                else {
    ...
    private static LongIoFunction<Decoder> getLongPromotionFunction(Schema writerSchema)
            throws AvroTypeException
    {
        if (writerSchema.getType().equals(Schema.Type.INT)) {
