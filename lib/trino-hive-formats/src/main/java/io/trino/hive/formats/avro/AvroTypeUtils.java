...
public final class AvroTypeUtils
{
        ...
        return customType.orElseGet(() -> switch (schema.getType()) {
            case RECORD -> {
                ...
                yield RowType.from(schema.getFields()
                        .stream()
                        .map(field -> new RowType.Field(Optional.of(field.name()), typeFromAvro(field.schema(), avroTypeManager, new HashSet<>(enclosingRecords))))
    ...
    protected static boolean isSimpleNullableUnion(Schema schema)
    {
    ...
    private static Schema unwrapNullableUnion(Schema schema)
    {
        ...
        return schema.getTypes().stream().filter(not(Schema::isNullable)).findFirst().orElseThrow(VerifyError::new);
    }
    ...
    private static RowType rowTypeForUnion(Schema schema, AvroTypeManager avroTypeManager, Set<Schema> enclosingRecords)
    {
        ...
        return RowType.from(
                ImmutableList.<RowType.Field>builder()
                        ...
                        .addAll(schema.getTypes().stream().filter(not(Schema::isNullable)).map(s -> RowType.field(UNION_FIELD_FIELD_PREFIX + fieldSuffix.getAndIncrement(), typeFromAvro(s, avroTypeManager, enclosingRecords))).collect(toImmutableList()))
                        .build());
    }
}
